...
function TodaySubjectModal({ modalId, nextSubjects, currentSubject }) {
    ...
    const hasSubjectToday = nextSubjects || currentSubject
                ...
                <div className={(nextSubjects?.length ? 'flex' : 'hidden') + ' w-11/12 h-full border-t-2 border-lightgray relative flex-col items-center'}>
                    ...
                    <div className='w-full max-h-48 flex my-6 overflow-auto scrollbar-none'>
                        ...
                        <div className='w-1/2 flex flex-col items-end'>
                            ...
                            {nextSubjects?.map(nextSubject => {
                               return <span key={nextSubject.id} className='text-lg mt-4'>{`${nextSubject.schedules[0].time.slice(0, -3)} - ${nextSubject.schedules[0].end.slice(0, -3)}`}</span>
                            })}
                        </div>
                    </div>
                </div>
                <div className={(hasSubjectToday?.length ? 'hidden' : 'flex') +" flex-col items-center justify-center p-6"}>
