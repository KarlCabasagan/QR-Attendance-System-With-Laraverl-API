...
function Dashboard({ userRole, navBtn, isLoggedIn, user, userSubjects, currentSubject }) {
    ...
    return (
        <div className={(isLoggedIn ? "flex" : "hidden") + " w-11/12 flex-col items-center relative"}>
            ...
            <div className="w-full flex flex-col items-center mt-64">
                {(userRole == 1 ) ? <StudentPage navBtn={navBtn} userRole={userRole} user={user} userSubjects={userSubjects} currentSubject={currentSubject} /> : <FacultyPage navBtn={navBtn} userRole={userRole} user={user} userSubjects={userSubjects} currentSubject={currentSubject} /> }
            </div>
        </div>
    )
}
